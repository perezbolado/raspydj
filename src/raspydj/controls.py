...
class Controller:
    def __init__(self, config:object):
        ...
        self.display.print_lines(['raspi-dj'])  

    def show_stats(self):
        cmd = "hostname -I | cut -d' ' -f1"
        ip = subprocess.check_output(cmd, shell=True).decode("utf-8")
        self.display.print_lines([ip])
    ...
    def update_nowplaying(self):
        ...
        if(track):
            self.display.print_lines(['Now Playing:', track['item']['name'], 'By: {}'.format(track['item']['artists'][0]['name'])])
...
#os.environ['SPOTIPY_CLIENT_ID'] = config['CREDENTIALS']['CLIENT_ID']wo
